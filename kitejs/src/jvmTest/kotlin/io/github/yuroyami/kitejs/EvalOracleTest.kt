...
class EvalOracleTest {
    ...
    @Test
    fun weakCollections() = check(listOf(
        "typeof WeakMap", "typeof WeakSet",
        "Object.prototype.toString.call(new WeakMap())", "Object.prototype.toString.call(new WeakSet())",
        "var k = {}; var m = new WeakMap(); m.set(k, 1); m.get(k)",
        "var k = {}; var m = new WeakMap(); m.set(k, 1); m.has(k)",
        "var m = new WeakMap(); m.has({})",
        "var k = {}; var m = new WeakMap(); String(m.get(k))",
        "var k = {}; var m = new WeakMap(); m.set(k, 1); m.delete(k)",
        "var k = {}; var m = new WeakMap(); m.delete(k)",
        "var k = {}; var m = new WeakMap(); m.set(k, 1); m.delete(k); m.has(k)",
        "var k = {}; var m = new WeakMap(); m.set(k, 1); m.set(k, 2); m.get(k)",
        "var k = {}; var m = new WeakMap(); m.set(k, null); String(m.get(k))",
        "var k = {}; var m = new WeakMap(); m.set(k, undefined); String(m.get(k))",
        "var k = {}; var m = new WeakMap(); m.set(k) === m",
        "var a = {}, b = {}; var m = new WeakMap(); m.set(a, 1); m.set(b, 2); m.get(a) + ',' + m.get(b)",
        "var m = new WeakMap([[{}, 1]]); typeof m",
        "var k = {}; var m = new WeakMap([[k, 7]]); m.get(k)",
        "var s = Symbol('x'); var m = new WeakMap(); m.set(s, 1); m.get(s)",
        "var m = new WeakMap(); try { m.set(1, 1) } catch(e) { e.name + ': ' + e.message }",
        "var m = new WeakMap(); try { m.set('a', 1) } catch(e) { e.name }",
        "var m = new WeakMap(); try { m.set(null, 1) } catch(e) { e.name }",
        "var m = new WeakMap(); try { m.set(undefined, 1) } catch(e) { e.name }",
        "var m = new WeakMap(); m.has(1)", "var m = new WeakMap(); m.delete(1)",
        "var m = new WeakMap(); String(m.get(1))",
        "try { WeakMap.prototype.get.call({}, {}) } catch(e) { e.name }",
        "try { WeakMap.prototype.set.call(new WeakSet(), {}, 1) } catch(e) { e.name }",
        "try { WeakMap() } catch(e) { e.name }",
        "var k = {}; var s = new WeakSet(); s.add(k); s.has(k)",
        "var s = new WeakSet(); s.has({})",
        "var k = {}; var s = new WeakSet(); s.add(k) === s",
        "var k = {}; var s = new WeakSet(); s.add(k); s.delete(k)",
        "var k = {}; var s = new WeakSet(); s.delete(k)",
        "var k = {}; var s = new WeakSet(); s.add(k); s.delete(k); s.has(k)",
        "var k = {}; var s = new WeakSet(); s.add(k); s.add(k); s.has(k)",
        "var k = {}; var s = new WeakSet([k]); s.has(k)",
        "var sym = Symbol('x'); var s = new WeakSet(); s.add(sym); s.has(sym)",
        "var s = new WeakSet(); try { s.add(1) } catch(e) { e.name + ': ' + e.message }",
        "var s = new WeakSet(); try { s.add('a') } catch(e) { e.name }",
        "var s = new WeakSet(); s.has(1)", "var s = new WeakSet(); s.delete(1)",
        "try { WeakSet() } catch(e) { e.name }",
        "try { WeakSet.prototype.add.call({}, {}) } catch(e) { e.name }",
        // Two different objects are two different keys, even when they look alike.
        "var m = new WeakMap(); m.set({a:1}, 'x'); String(m.get({a:1}))",
        "var s = new WeakSet(); s.add({a:1}); s.has({a:1})",
        "var f = function(){}; var m = new WeakMap(); m.set(f, 1); m.get(f)",
        "var a = []; var m = new WeakMap(); m.set(a, 1); m.get(a)",
        "'set' in WeakMap.prototype", "'add' in WeakSet.prototype", "'size' in WeakMap.prototype",
