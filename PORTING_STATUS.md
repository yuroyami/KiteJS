...
| Function objects (BaseFunction, LambdaFunction, LambdaConstructor, TopLevel) | 🟡 | The classes are ported and the object model builds on them. The `Function` constructor itself, `apply`, `call`, `bind` and the live `arguments` object wait on the classes they need; each spot is marked in the source |
